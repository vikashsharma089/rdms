var DISTRIBUTE = function() {
	this.show = function() {
		...
		content = content + '</div><div class="dataTable-search"><input class="dataTable-input" id="searchId" onkeyup="distribute.searchCard(this)" placeholder="Search by ration card" type="text"></div></div>'
	...
	this.loadAndAppendPendingMonth = function() {
		var response = ajax.get("/month/loadPendingMonth/"+villageId);
			...
			for (var i = 0; i < obj.length; i++) {
				...
				var map = new Object(); // or var map = {};
				    if(jsonData[j].quantity !== 0){
				       var config = ruleSetting.get(jsonData[j].stockItem.itemName);
				        var totalQuantity = 0;
				       if(config.rationCardType == "all"){
				        
				        totalQuantity = distribute.getTotalCalculatedKg(config,unit)
				       }
				       if(config.rationCardType === obj[i].cartType ){

...
- Added new users interface `enable`, `disable`, and `isDisabled` methods. #TINY-6397
...
- Added new local upload feature this allows the users to upload images directly from the image dialog.
...
- Fixed bug where an exception would be thrown if the users selected a video element and then moved the focus outside the editor.
...
- Fixed so adjacent pre blocks gets joined into one pre block since that seems like the users intent.
...
- Added new notification API. Lets you display floating notifications to the end users.
...
- Fixed bug where inline editing of a table as the root element could let the users delete that table.
- Fixed bug where inline editing of a table as the root element wouldn't properly handle enter key.
- Fixed bug where inline editing of a table as the root element would normalize the selection incorrectly.
- Fixed bug where inline editing of a list as the root element could let the users delete that list.
- Fixed bug where inline editing of a list as the root element could let the users split that list.
...
- Added new link_assume_external_targets option to prompt the users to prepend http:// prefix if the supplied link does not contain a protocol prefix.
...
- Fixed bug where delete/backspace on WebKit/Blink would remove span elements created by the users.
...
- Fixed bug where pasting would sometimes scroll to the top of page if the users was using the autoresize plugin.
